...
@Component({
  selector: 'app-upview-fb',
  templateUrl: './upview-fb.component.html',
  styleUrls: ['./upview-fb.component.css']
})
export class UpviewFbComponent {
  ...
  private GetAccountMenuID() {
    this.ever.get('Account/Menu').subscribe( red => {
      this.ever.get(`Id?Id=${red[0].service[1].id}`).subscribe( res => {
        this.Orderform1 = this.formBuilder.group({
          id: [''],
          sid: [res.id],
          url: [''],
      ...
      this.facebook = red[0].service[1].title;
      this.dongia = red[0].service[1].price; 
    })
  }
  thanhtoan() {
